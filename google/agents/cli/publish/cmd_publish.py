...
def get_agent_runtime_metadata(agent_runtime_id: str) -> tuple[str | None, str | None]:
    ...
    try:
        ...
        console_err.print(f"Warning: Could not fetch metadata from Agent Runtime: {e}")
        return None, None
...
def _list_gemini_enterprise_apps(project_id: str | None) -> None:
    """List Gemini Enterprise apps across all locations for the given project."""
    resolved_project_id = project_id or get_current_project_id()
...
def register_gemini_enterprise(
    ...
    """
    if list_apps:
        _list_gemini_enterprise_apps(project_id)
        return

    display_welcome_banner(register_mode=True, quiet=not interactive)
