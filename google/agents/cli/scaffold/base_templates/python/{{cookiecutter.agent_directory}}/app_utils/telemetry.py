...
def setup_telemetry() -> str | None:
    ...
    # Set up custom A2A instrumentation
    _setup_custom_a2a_instrumentation(span_exporter)

    return bucket


def _setup_custom_a2a_instrumentation(exporter: SpanExporter) -> None:
    """Set up custom A2A instrumentation."""
    try:
        Traceloop.init(
...
def _setup_genai_telemetry(bucket: str) -> SpanExporter:
    ...
    log_name = os.environ.get("GOOGLE_CLOUD_DEFAULT_LOG_NAME", "custom-a2a-otel")
